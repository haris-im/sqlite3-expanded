...
void Statement::Work_Run(napi_env e, void* data) {
    ...
    if (stmt->Bind(baton->parameters)) {
        const char* expanded = sqlite3_expanded_sql(stmt->_handle);
        if (expanded) {
            baton->expanded_sql = std::string(expanded);
...
void Statement::Work_AfterRun(napi_env e, napi_status status, void* data) {
    ...
    if (stmt->status != SQLITE_ROW && stmt->status != SQLITE_DONE) {
        ...
        if (IS_FUNCTION(cb)) {
            ...
            result.Set("lastID", Napi::Number::New(env, baton->inserted_id));
            result.Set("changes", Napi::Number::New(env, baton->changes));
            result.Set("expandedSql", Napi::String::New(env, baton->expanded_sql));
